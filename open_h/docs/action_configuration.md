...
Note that GR00T-H was trained with `state_dropout_prob_per_embodiment: 1.0` for all embodiments in `gr00t_h_config.yaml`. This replaces encoded state features with a learned mask token, making the policy effectively vision-only for those embodiments. The `pass_through_keys` mechanism is separate from state dropout — it controls which keys reach the encoder, while state dropout controls whether the encoder output is used.
...
2. **Add a projector index** in `gr00t/model/gr00t_n1d7/processing_gr00t_n1d7.py` (maps your tag to a unique projector slot).
