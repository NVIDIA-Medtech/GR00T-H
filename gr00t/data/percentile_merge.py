...
def merge_piecewise_linear_quantiles(
    ...
    if target_probs.ndim != 1:
        raise ValueError(
            "Quantile merge target_probs must be a 1D array. "
            f"Got shape {target_probs.shape}."
        )
    ...
    if np.any(np.diff(target_probs) < 0):
        raise ValueError(
            "Quantile merge target_probs must be non-decreasing. "
            f"Got {target_probs}."
        )
    if np.any((target_probs < 0.0) | (target_probs > 1.0)):
        raise ValueError(
            "Quantile merge target_probs must be within [0, 1]. "
            f"Got {target_probs}."
        )
