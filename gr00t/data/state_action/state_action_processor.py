...
class StateActionProcessor:
    ...
    def apply_action(
        ...
        for idx, joint_group in enumerate(modality_keys):
            ...
            normalization_type = action_config.normalization_type if action_config else "temporal_meanstd"
                    ...
                    if has_motion_scaling:
                        if action_config.translation_scaling_key:
                            trans_scales = self._extract_scaling_values(
                                state, action_config.translation_scaling_key,
                                num_samples, is_batched, embodiment_tag,
                            )
                        if action_config.rotation_scaling_key:
                            rot_scales = self._extract_scaling_values(
                                state, action_config.rotation_scaling_key,
                                num_samples, is_batched, embodiment_tag,
                            )
    ...
    def _extract_scaling_values(
        ...
        if (
            is_batched
            and scale_values.ndim > 0
            and scale_values.shape[0] == num_samples
        ):
            return np.array(
                [
                    float(np.asarray(scale_values[i]).reshape(-1)[0])
